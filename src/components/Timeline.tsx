...
export function Timeline() {
  const groupedItems = Array.from(
    timelineItems.reduce((map, item) => {
      const existingItems = map.get(item.type) ?? [];
      existingItems.push(item);
      map.set(item.type, existingItems);
      return map;
    ...
    <div className="relative space-y-4 before:absolute before:left-4 before:top-2 before:h-[calc(100%-1rem)] before:w-px before:bg-white/10">
      {groupedItems.map(([type, items]) => (
        <motion.article
          ...
          className="relative grid gap-4 rounded-lg border border-white/10 bg-white/[0.035] p-5 pl-12"
        >
          ...
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-300">{type}</p>

          <div className="grid gap-4">
            {items.map((item, index) => (
              <div
                key={`${item.type}-${item.title}`}
                className={index === 0 ? "grid gap-2" : "grid gap-2 border-t border-white/10 pt-4"}
              >
                <div>
